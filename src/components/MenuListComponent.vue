<template>
	<main-master-page>
		<div class="wrapper">
			<h2 class="menu__title">Меню - <span>{{ listTitle }}</span></h2>
			<router-link class="menu__link" :to="{name: 'menu'}">
				<font-awesome-icon :icon="['fas', 'left-long']" /> &nbsp;
				Назад до категорій
			</router-link>
			<div class="menu__container">
				<menu-item v-for="dish in dishesFilteredByCategory" :key="dish.id" :dish="dish"/>
			</div>
		</div>
	</main-master-page>
</template>
...
.menu__title {
	margin: 150px 0 .5em;
	font-size: clamp(1.25rem, 0.667rem + 2.917vw, 3rem);
	font-weight: 600;
	span {
		font-size: clamp(1.125rem, 0.667rem + 2.292vw, 2.5rem);
	}
}
.menu__link {
	display: block;
